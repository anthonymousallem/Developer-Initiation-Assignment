...
namespace BLC
{
    ...
    public partial class BLC : IDisposable
    {
        ...
        public enum Enum_BR_Codes
        {
            ...
            BR_0000,  // Uniqueness Violation                      s
        ...
        {
            #region Declaration And Initialization Section.
            this.OnPostEvent_Edit_Admin += new PostEvent_Handler_Edit_Admin(UserCreation);
            this.OnPreEvent_Edit_Admin += new PreEvent_Handler_Edit_Admin(PasswordValidation);
            #endregion
            #region Body Section.
            #endregion
        }

        private bool EmailChecker(string emailAdress)
        {
            try
            {
                var email = new System.Net.Mail.MailAddress(emailAdress);
                return true;
            }
        ...
        private void PasswordValidation(Admin i_Admin, Enum_EditMode i_Enum_EditMode)
        {
            if (!EmailChecker(i_Admin.EMAIL))
            {
                throw new Exception(" Enter a valid email");
            }

            if (i_Admin.PASSWORD.Length < 7)
            {
                throw new Exception(" Password should be at leasst 7 characters");
            }
        }
        ...
        private void UserCreation(Admin i_Admin, Enum_EditMode i_Enum_EditMode)
        {
            if (i_Enum_EditMode == Enum_EditMode.Add)
            {
                ...
                User newUser = new User
                {
                    ...
                    USER_ID = -1,
                    USER_TYPE_CODE = "001",
                    USERNAME = i_Admin.EMAIL,
                    PASSWORD = oMiniCryptoHelper.Encrypt(i_Admin.PASSWORD),
                    IS_ACTIVE = true
                };
                Edit_User(newUser);
            }
